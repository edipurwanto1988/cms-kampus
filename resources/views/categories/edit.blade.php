...
<div class="max-w-6xl mx-auto">
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600">Update category details with multilingual content support</p>
    </div>
            ...
            <div class="space-y-6">
                            ...
                            <div class="space-y-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input type="text"
                                               ...
                                               value="{{ old('translations.' . $language->code . '.name', $category->translations->where('locale', $language->code)->first()->name ?? '') }}"
                                    ...
                                    <div>
                                        ...
                                        <input type="text"
                                               ...
                                               value="{{ old('translations.' . $language->code . '.slug', $category->translations->where('locale', $language->code)->first()->slug ?? '') }}"
                                ...
                                <div>
                                    ...
                                    <textarea id="description_{{ $language->code }}"
                                              ...
                                              placeholder="Category description">{{ old('translations.' . $language->code . '.description', $category->translations->where('locale', $language->code)->first()->description ?? '') }}</textarea>
                ...
                <div>
                    ...
                    @if($category->imageMedia)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $category->imageMedia->path) }}" alt="Current image" class="max-w-xs h-auto rounded">
                    ...
                    <p class="mt-1 text-xs text-gray-500">Upload new image to replace. System will automatically create thumbnail based on settings.</p>
                    @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                        <i class="fas fa-save mr-2"></i>Update Category with All Translations
                    </button>
                </div>
            </div>
        </form>
